...
def parse_tair_gff_file(chrom=None, reg_start_pos=None, reg_end_pos=None, only_genes=False):
    ...
    if os.path.isfile(pickled_filename) and chrom is None:
        ...
        for line in lines:
            ...
            info_list = l[8].split(';')
            chromosome = l[0][3]
            if chrom is not None and str(chrom) != chromosome:
                continue
            start_pos = int(l[3])
            end_pos = int(l[4])
            strand = l[6]
        ...
        elif reg_start_pos is not None and reg_end_pos is not None:
...
#    file_prefix = '%s%d/kinship_%s_%s' % (env['cm_dir'], call_method_id, method, data_format)
...
#    id_sd = parse_snp_data('/net/gmi.oeaw.ac.at/gwasapp/gwas-frontend/dataset/80/indels_svs_binary.csv', format='binary')
